...
class MessageService:
    ...
    async def delayed_message(self, start_time: datetime, sender: str):
        last_message = await self.get_last_message(sender)

        if last_message.created_at >= start_time + datetime.timedelta(minutes=5):
            return None
        ...
        await self.chat_api.send_message(sender, response_text)
    ...
    async def get_last_message(self, sender: str) -> Messages | None:
        query = (
            select(Messages)
            .where(Messages.sender == sender)
            .order_by(Messages.created_at.desc())
